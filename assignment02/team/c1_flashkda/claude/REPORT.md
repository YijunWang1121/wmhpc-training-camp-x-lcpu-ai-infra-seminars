...
这个场景)上比原版快 **1.4~2.7×**,精度与原版相同;B·H ≥ 48 时不启用(门控后无任何形状变慢)。在这个新 workload
上,SM100 的 tcgen05 第一次出现正收益:组摘要的稠密 compose 步比 K2 的 chunk 快 1.9~3.0×(已实测原语),整条流水线
纸面还能再 ~2×。

**收益条件表(E11,47 个形状 × 5 轮交替计时,std ≤ 1.5%,详表见 §3.3):**

| 原版 K2 的 CTA 数 B·H | 序列长度 | 分层扫描加速(几何均值 / 范围) | 判定 |
|--:|--|--:|--|
| 12(TP8) | 4K → 128K | **2.16×**(1.48× → 2.66×,随 T 单调上升) | 赢 |
| 16 | 4K → 128K | **1.86×**(1.42× → 2.11×) | 赢 |
| 24 | 4K / ≥16K | 0.98× / 1.21~1.34× | ≥16K 赢 |
| 32 | 4K / ≥16K | 0.92~0.98× / 1.03~1.19× | ≥16K 略赢 |
| 48 / 64 | 任意 | 0.67~0.79× / 0.61~0.68× | 输 → 门控走原版(1.00×) |
| 128 / 256 | 任意 | 0.39× / 0.33× | 输 → 门控走原版 |

再叠加 SM100(纸面,原语已实测):B=1,H=12,T=8K 从 710 µs → ~355 µs(对原版 ~3.9×,§4)。
...
# srun -G 1 bash -c 'PYTHONPATH=$C1:exp $PY exp/e11_matrix.py 5'                    # -> logs/e11_matrix_srun.txt
