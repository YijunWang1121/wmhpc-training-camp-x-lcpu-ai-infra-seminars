...
**精度(§3.4、§3.6 E17):** 树版对 fp64 真值的误差与原版逐项相同(如 T=8K、组间携带 70% 状态时 out 6.97e-3 vs
6.95e-3),不随 T 增长(1K→16K 平),分段调用零额外损失;W=1 回看版在判据成立的全部 18 个 (lb,G,T) 格里对 fp64 的
误差与原版逐格相同,lb=−5/−1 时与原版逐位一致(2500 万元素 0 个不同);判据不成立时(lb=−0.01、G=32)误差 +9~13%,
W=2 即恢复。
...
**精度(E17,对 fp64 真值,`exp/e9_hier.py precision_lb`,`logs/e17_precision_lb_srun.txt`,H=4,rel_rms):**
把 W=1 放到 §3.4 同一套协议下——原版 / 树版 / W=1 / W=2 四者各自对 fp64 朴素递推的误差,随 T(1K→16K)与 lb 扫:

| lb | G | 衰减判据(最弱组) | out 对 fp64:原版 / 树版 / W=1 / W=2 | hT 对 fp64:原版 / W=1 | W=1 对原版 out / hT |
|--:|--:|--:|--|--|--|
| −5 | 32 / 64 | 0 | 5.20~5.24e-3 / 同 / **同** / 同(T=1K~16K) | 4.2~5.1e-3 / **同** | **0 / 0(逐位一致)** |
| −1 | 32 / 64 | 10⁻¹⁰⁴~10⁻²¹⁷ | 5.47~5.52e-3 / 同 / **同** / 同 | 4.5~4.7e-3 / **同** | **0 / 0(逐位一致)** |
| −0.1 | 32 | 2~5e-11 | 5.93~5.99e-3 / 同 / **同** / 同 | 5.0e-3 / **同** | 6~9e-5 / 0 |
| −0.1 | 64 | 10⁻²¹ | 5.96~5.98e-3 / 同 / **同** / 同 | 5.0e-3 / **同** | 0 / 0 |
| −0.01 | 32 | **0.085~0.093(不成立)** | 6.94e-3 / 6.97e-3 / **7.56~7.84e-3** / 6.98e-3 | 6.1e-3 / 6.1e-3 | 6e-3 / 5e-3 |
| −0.01 | 64 | 7~8e-3(不成立) | 6.94~6.96e-3 / 同 / 6.93~6.96e-3 / 同 | 6.1e-3 / 6.1e-3 | 5e-3 / 5e-3 |

- 判据成立的全部 18 格(lb=−5/−1/−0.1):W=1 对 fp64 的误差与原版**逐格相同到三位有效数字**,不随 T 增长;
  lb=−5/−1 时 W=1 与原版**逐位一致**(另测 B=2,H=12,T=8192,G=32/64,lb=−5:2500 万个 out 元素、39 万个 hT 元素,
  不同元素数 0)。
- 判据不成立的 6 格(lb=−0.01,人为极弱衰减):G=32 时 W=1 对 fp64 的 out 误差从 6.94e-3 升到 7.56~7.84e-3(+9~13%),
  hT 不受影响;**W=2 即回到原版水平**(6.95~6.98e-3)。G=64 时判据仍不成立(7e-3 > 2⁻⁹)但 W=1 已与原版相同——判据是
  保守的(按最弱一维、最弱一组判),宁可多回看一层。
- 分段调用(W=1 跑 T1 的末状态作为下一段 h0):lb=−1 与一次原版逐位一致,lb=−0.1 差 6e-5(bf16 舍入),对 fp64 误差
  与原版相同(5.47e-3 / 5.96e-3)。

结论:**W=1 在判据成立时不改变数值结果**;判据由 K1 已有的 gate cumsum 直接算出,失败时按 (b,h) 加大 W(W=2 已够)
或回退树版。
...
# srun -G 1 bash -c 'PYTHONPATH=$C1:exp $PY exp/e9_hier.py precision_lb'             # -> logs/e17_precision_lb_srun.txt(W=1/W=2 对 fp64 真值)
